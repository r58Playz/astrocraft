...
class Model(object):
    ...
    def _initialize(self):
        ...
        if self.saveModule.hasSaveGame() == True:
            ...
            for x in list(range(-n, n + 1, s)):
                for z in list(range(-n, n + 1, s)):
                    # create a layer stone an grass everywhere.
                    self.add_block((x, y - 2, z), GRASS, immediate=False)
                    self.add_block((x, y - 3, z), STONE, immediate=False)
                    if x in (-n, n) or z in (-n, n):
                        # create outer walls.
                        for dy in list(range(-2, 3)):
                            self.add_block((x, y + dy, z), STONE, immediate=False)

            # generate the hills randomly
            o = n - 10
            for _ in list(range(120)):
                ...
                for y in list(range(c, c + h)):
                    for x in list(range(a - s, a + s + 1)):
                        for z in list(range(b - s, b + s + 1)):
    ...
    def hit_test(self, position, vector, max_distance=8):
        ...
        dx, dy, dz = vector
        previous = None
        for _ in list(range(max_distance * m)):
            key = normalize((x, y, z))
    ...
    def change_sectors(self, before, after):
        ...
        after_set = set()
        pad = 4
        for dx in list(range(-pad, pad + 1)):
            for dy in [0]:  # list(range(-pad, pad + 1):
                for dz in list(range(-pad, pad + 1)):
...
class Window(pyglet.window.Window):
    ...
    def collide(self, position, height):
        ...
        for face in FACES:  # check all surrounding blocks
            for i in list(range(3)):  # check each dimension independently
                if not face[i]:
                    continue
                d = (p[i] - np[i]) * face[i]
                if d < pad:
                    continue
                for dy in list(range(height)):  # check each height
                    op = list(np)
                    op[1] -= dy
                    op[i] += face[i]
                    op = tuple(op)
